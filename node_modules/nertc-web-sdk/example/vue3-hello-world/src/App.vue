...
const vueApp =  {
  ...
  methods: {
    joinChannel:async function(){
      console.log(`joinChannel`, this.client, this.localStream)
      rtc.client = this.client
      rtc.localStream = this.localStream
      rtc.client._params.appkey = this.appkey
  ...
  data: ()=>{
    const client = NERTC.createClient({appkey: 'abcabc'})
    const localStream = NERTC.createStream({
      client,
      audio: true,
      video: true
    })
    client.__v_skip = true
    localStream.__v_skip = true
    return {
      client,
      localStream,

...
declare interface Client {
  ...
  /**
   * 开启实时字幕
   * 需要在加入频道后调用`client.startAsrCaptions()`方法。调用后，可收到`asr-captions`事件。
   * @return {boolean}
   *
   *
   * @example
  ...
  /**
   * 关闭实时字幕
   * 需要在开启实时字幕后调用`client.stopAsrCaptions()`方法。调用后，不再收到`asr-captions`事件。
   */
   ...
   */
  on(event: 'asr-captions', callback: (result: Array<AsrCaption>) => void): void

  /**
   * `device-open-fail` 摄像头打开失败，当触发该事件时，会使用默认参数{video: true}尝试重新打开摄像头。
   *
   * @param constraint 摄像头参数。
   */
  on(
    event: 'device-open-fail',
    callback: (constraint: { video: GUMVideoConstraints }) => void
  ): void

  /**
   * `displaySurfaceError` 屏幕共享选择的区域不合法。（加入房间前修改 NERTC.getParameters().forceDisplaySurface 以选择屏幕共享区域。当触发该事件时，会自动关闭屏幕共享）
   * forceDisplaySurface: 'default' | 'browser' | 'window' | 'monitor', 默认为'default'
   */
  on(event: 'displaySurfaceError', callback: () => void): void

  /**
   * `displaySurfaceUnknown` 屏幕共享选择的区域未知。（加入房间前修改 NERTC.getParameters().forceDisplaySurface 以选择屏幕共享区域.当无法判断用户选择的屏幕共享区域时，会触发该事件，可通过该事件回调主动关闭屏幕共享）
   * forceDisplaySurface: 'default' | 'browser' | 'window' | 'monitor', 默认为'default'
   *
   * @param value 屏幕共享track的label值
   */

...
function parseInlineStyleSync(input, options = {}) {
    ...
    if (typeof expandDeclaration === 'function') {
        declarations.forEach((declaration) => {
            expandedDeclarations.push(...visit(declaration, expandDeclaration));
        });
    }
    else {
    ...
    if (typeof normalizeDeclaration === 'function') {
        expandedDeclarations.forEach((declaration) => {
            normalizedDeclarations.push(...visit(declaration, normalizeDeclaration));
        });
    }
    else {
        normalizedDeclarations.push(...expandedDeclarations);
    }
    const styleEntries = normalizedDeclarations.map(({ prop, value }) => `[${JSON.stringify(prop)}, ${JSON.stringify(value)}]`);
    if (styleEntries.length === 0) {
        return;
    }
    return `new Map([${styleEntries.join(', ')}])`;
}
function visit(declaration, transformDecl) {
    let removed = false;
    let replaced = false;
    const result = [];
...
exports.parse = parse;
exports.parseInlineStyleSync = parseInlineStyleSync;

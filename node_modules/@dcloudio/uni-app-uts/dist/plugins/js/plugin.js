...
function createUniAppJsEnginePlugin(platform) {
    return function uniAppJsEnginePlugin() {
        ...
        if (process.env.UNI_UTS_PLATFORM === 'app-harmony') {
            const esmFile = path_1.default.resolve(process.env.UNI_INPUT_DIR, '.esm');
            if (fs_extra_1.default.existsSync(esmFile)) {
                process.env.UNI_APP_OUTPUT_FORMAT = 'esm';
                if (fs_extra_1.default.readFileSync(esmFile, 'utf-8').trim() === 'dynamic') {
                    // 动态导入
